...
function getDmg(spellname,target,myHero,stagedmg,spelllvl)
	...
	local ar = GetArmor(myHero)
	local mmana = GetMaxMana(myHero)
	local mana = GetCurrentMana(myHero)
	local mhp = GetMaxHP(myHero)
	local tap = GetBonusAP(target)
	...
	elseif spellname == "Q" or spellname == "W" or spellname == "E" or spellname == "R" or spellname == "P" or spellname == "QM" or spellname == "WM" or spellname == "EM" then
		...
		elseif name == "Ahri" then
			if spellname == "Q" then DmgM = (25*Qlvl+15+.35*ap)*(stagedmg1+stagedmg3) DmgT = (25*Qlvl+15+.35*ap)*(stagedmg2+stagedmg3)
			elseif spellname == "W" then DmgM = math.max(25*Wlvl+15+.4*ap,(25*Wlvl+15+.4*ap)*1.6*stagedmg3) 
			elseif spellname == "E" then DmgM = 30*Elvl+30+.35*ap 
			elseif spellname == "R" then DmgM = 40*Rlvl+30+.3*ap 
			end
		elseif name == "Akali" then
			if spellname == "P" then DmgM = (6+.1666667*ap)*ad/100
			elseif spellname == "Q" then DmgM = math.max((20*Qlvl+15+.4*ap)*stagedmg1,(25*Qlvl+20+.5*ap)*stagedmg2,(45*Qlvl+35+.9*ap)*stagedmg3) 
	...
	elseif (spellname == "AD") then
		TrueDmg = myHero:CalcDamage(target,ad)
	...
	elseif (spellname == "HXG") then
		TrueDmg = myHero:CalcMagicDamage(target,150+.4*ap)
	elseif (spellname == "BWC") then
		TrueDmg = myHero:CalcMagicDamage(target,100)
	elseif (spellname == "NTOOTH") then
		TrueDmg = myHero:CalcMagicDamage(target,15+.15*ap)
	elseif (spellname == "WITSEND") then
		TrueDmg = myHero:CalcMagicDamage(target,42)
	elseif (spellname == "SHEEN") then
		TrueDmg = myHero:CalcDamage(target,ad-bad) 
	elseif (spellname == "TRINITY") then
		TrueDmg = myHero:CalcDamage(target,2*(ad-bad)) 
	elseif (spellname == "LICHBANE") then
		TrueDmg = myHero:CalcMagicDamage(target,.75*(ad-bad)+.5*ap) 
	elseif (spellname == "LIANDRYS") then
		TrueDmg = myHero:CalcMagicDamage(target,.06*thp) 
	elseif (spellname == "STATIKK") then
		TrueDmg = myHero:CalcMagicDamage(target,100)
	elseif (spellname == "ICEBORN") then
		TrueDmg = myHero:CalcDamage(target,1.25*(ad-bad)) 
	elseif (spellname == "TIAMAT") then
		TrueDmg = myHero:CalcDamage(target,.6*ad) 
	elseif (spellname == "HYDRA") then
		TrueDmg = myHero:CalcDamage(target,.6*ad) 
	elseif (spellname == "RUINEDKING") then
		TrueDmg = math.max(myHero:CalcDamage(target,.08*thp)*(stagedmg1+stagedmg3),myHero:CalcDamage(target,math.max(.1*tmhp,100))*stagedmg2) 
	elseif (spellname == "MURAMANA") then
		TrueDmg = myHero:CalcDamage(target,.06*mana)
	elseif (spellname == "HURRICANE") then
		TrueDmg = myHero:CalcDamage(target,10+.5*ad) 
	elseif (spellname == "SUNFIRE") then
		TrueDmg = myHero:CalcMagicDamage(target,25+lvl) 
	elseif (spellname == "LIGHTBRINGER") then
		TrueDmg = myHero:CalcMagicDamage(target,100) 
	elseif (spellname == "MOUNTAIN") then
		TrueDmg = myHero:CalcMagicDamage(target,.3*ap+ad)
	end	
