...
for i,enemy in pairs(GoS:GetEnemyHeroes()) do
      if ChillingSmite and AutoSmiteMenu.Enabled:Value() then
                  if CanUseSpell(myHero, ChillingSmite) == READY and 20+8*GetLevel(myHero) > GetCurrentHP(enemy)+GetDmgShield(enemy) and GoS:GetDistanceSqr(GetOrigin(enemy)) < 500*500 then
                  CastTargetSpell(enemy, ChillingSmite)
                  end
      elseif Ignite and AutoIgniteMenu.Enabled:Value() then
                  if CanUseSpell(myHero, Ignite) == READY and 20*GetLevel(myHero)+50 > GetCurrentHP(enemy)+GetDmgShield(enemy)+GetHPRegen(enemy)*2.5 and GoS:GetDistanceSqr(GetOrigin(enemy)) < 600*600 then
                  CastTargetSpell(enemy, Ignite)
                  end
      elseif DuelSmite and AutoSmiteMenu.Enabled:Value() then
                  if CanUseSpell(myHero, DuelSmite) == READY and 54+6*GetLevel(myHero) > GetCurrentHP(enemy)+GetDmgShield(enemy)+GetHPRegen(enemy)*3 and GoS:GetDistanceSqr(GetOrigin(enemy)) < 500*500 then
