...
class 'IACR' -- {
  ...
  function IACR:__init(bool)
    IWalkTarget = nil
    myHero = GetMyHero()
    myHeroName = GetObjectName(myHero)
    waitTickCount = 0
   IWalkConfig = menu.addItem(SubMenu.new("Inspired's Auto Carry : Reborn"))
    self.move = true
    self.aa = true
  ...
  function IACR:OnLoop()
        if DmgCalc.getValue() then self:DmgCalc() end
        if waitTickCount > GetTickCount() then return end
        self:IWalk()
  end

  function IACR:DmgCalc()
  ...
  function IACR:PossibleDmg(unit)
    ...
    if myHeroName == "Riven" then
      ...
      if (CanUseSpell(myHero, _R) == READY or GetCastName(myHero, _R) ~= "RivenFengShuiEngine") and IWalkConfig.R then
        ...
        if rdmg > cthp and ValidTarget(unit, 800) and GetCastName(myHero, _R) ~= "RivenFengShuiEngine" and Combo.getValue() then 
  ...
  function IACR:IWalk()
    if LastHit.getValue() or LaneClear.getValue() or Harass.getValue() then
      for _,k in pairs(GetAllMinions(MINION_ENEMY)) do
        ...
        if dmg > hp then
          if (LastHit.getValue() or LaneClear.getValue() or Harass.getValue()) and IsInDistance(k, self.myRange) and GetTickCount() > self.orbTable.lastAA + self.orbTable.animation then
            AttackUnit(k)
            return
          end
        end
    ...
    if Combo.getValue() or Harass.getValue() or LastHit.getValue() or LaneClear.getValue() then
      self:DoWalk()
    end
  end

  function IACR:DoWalk()
    ...
    if not LastHit.getValue() and ValidTarget(unit, self.myRange) and GetTickCount() > self.orbTable.lastAA + self.orbTable.animation and self.aa then
      AttackUnit(unit)
    elseif GetTickCount() > self.orbTable.lastAA + self.orbTable.windUp and self.move then
      if GetRange(myHero) < 450 and IWalkConfig.wtt and unit and GetObjectType(unit) == GetObjectType(myHero) and ValidTarget(unit, self.myRange) then
        ...
        if Combo.getValue() and self.gapcloserTable[myHeroName] and ValidTarget(unit, self.myRange + 250) and IWalkConfig[str[self.gapcloserTable[myHeroName]].."g"] and CanUseSpell(myHero, gapcloserTable[myHeroName]) == READY then
          local unitPos = GetOrigin(unit)
  ...
  function IACR:GetIWalkTarget()
    return IWalkTarget
  end

  function IACR:ProcessSpell(unit, spell)
      ...
      if unit == myHero then
        if (spell.name:lower():find("attack") or (self.isAAaswellTable[myHeroName] and self.isAAaswellTable[myHeroName] == spell.name)) then
          ...
          DelayAction(function() 
                              if Combo.getValue() and ValidTarget(IWalkTarget, self.myRange) then 
  ...
  function IACR:WindUp(unit)
    ...
    if self.aaResetTable[myHeroName] then
      for _,k in pairs(self.aaResetTable[myHeroName]) do
        if CanUseSpell(myHero, k) == READY and GetDistanceSqr(GetOrigin(unit)) < self.myRange * self.myRange then
          self.orbTable.lastAA = 0
          CastSpell(k)
          return true
        end
    ...
    if self.aaResetTable2[myHeroName] then
      for _,k in pairs(self.aaResetTable2[myHeroName]) do
        if CanUseSpell(myHero, k) == READY and GetDistanceSqr(GetOrigin(unit)) < self.myRange * self.myRange and (not myHeroName == "Quinn" or CanUseSpell(myHero, _E) ~= READY) then
          local unitPos = GetOrigin(unit)
  ...
  function IACR:MakeMenu()	
    DmgCalc = IWalkConfig.addItem(MenuBool.new("Damage Calc", true))
    LastHit = IWalkConfig.addItem(MenuKeyBind.new("LastHit", 88))
    Harass = IWalkConfig.addItem(MenuKeyBind.new("Harass", 67))
    LaneClear = IWalkConfig.addItem(MenuKeyBind.new("LaneClear", 86))
    Combo = IWalkConfig.addItem(MenuKeyBind.new("Combo", 32))
  end

  function IACR:IsWindingUp()
    return GetTickCount() <= self.orbTable.lastAA + self.orbTable.windUp
  end

  function IACR:SetMove(bool)
    self.move = bool
  end

  function IACR:SetAA(bool)
    self.aa = bool
  end

  function IACR:SetOrb(bool)
    self.aa = bool
    self.move = bool
  end
