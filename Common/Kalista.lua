   ...
   if IOW:Mode() == "Harass" and 100*GetCurrentMana(myHero)/GetMaxMana(myHero) >= KalistaMenu.Harass.Mana:Value() then
	...
	if IOW:Mode() == "Combo" then
	   ...
	   if Ignite and KalistaMenu.Misc.AutoIgnite:Value() then
           ...
           if CanUseSpell(myHero, _E) == READY and GoS:ValidTarget(enemy, 1000) and KalistaMenu.Killsteal.E:Value() and GetCurrentHP(enemy)+GetDmgShield(enemy) < Edmg(enemy) then
	   CastSpell(_E)
