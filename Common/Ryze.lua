...
function Combo()
  if IOW:Mode() == "Combo" then
    ...
    if GotBuff(myHero, "ryzepassivecharged") > 0 and CanUseSpell(myHero, _R) ~= READY then
	  ...
	  if SpellQREADY and RyzeMenu.Combo.Q:Value() then 
	  if GotBuff(target, "snare") > 0 or GotBuff(target, "stun") > 0 or GotBuff(target, "suppression") then
          CastSkillShot(_Q,targetPos.x,targetPos.y,targetPos.z)
	  end
	  elseif SpellQREADY and QPred.HitChance == 1 and RyzeMenu.Combo.Q:Value() then
	  CastSkillShot(_Q,QPred.PredPos.x,QPred.PredPos.y,QPred.PredPos.z)  
	  elseif SpellQREADY and RyzeMenu.Combo.Q:Value() then
    ...
    elseif GotBuff(myHero, "ryzepassive") > 0 then
	  ...
	  elseif SpellQREADY and QPred.HitChance == 1 and RyzeMenu.Combo.Q:Value() then
	  CastSkillShot(_Q,QPred.PredPos.x,QPred.PredPos.y,QPred.PredPos.z)
	  elseif SpellQREADY and GotBuff(myHero, "ryzepassivestack") > 3 or GotBuff(myHero, "ryzepassivecharged") > 0 and RyzeMenu.Combo.Q:Value() then
	  CastSkillShot(_Q,Q2Pred.PredPos.x,Q2Pred.PredPos.y,Q2Pred.PredPos.z)
	  end
...
function Killsteal()
	for i,enemy in pairs(GoS:GetEnemyHeroes()) do
		...
		CastSkillShot(_Q,QPred.PredPos.x,QPred.PredPos.y,QPred.PredPos.z)
		elseif SpellWREADY and GoS:ValidTarget(enemy,GetCastRange(myHero,_W)) and RyzeMenu.Killsteal.W:Value() and GetCurrentHP(enemy)+GetMagicShield(enemy)+GetDmgShield(enemy) < CalcDamage(myHero, enemy, 0, 60+20*GetCastLevel(myHero,_W)+0.4*GetBonusAP(myHero)+0.025*GetMaxMana(myHero) + ExtraDmg) then
		CastTargetSpell(enemy, _W)
		elseif SpellEREADY and GoS:ValidTarget(enemy,GetCastRange(myHero,_E)) and RyzeMenu.Killsteal.E:Value() and GetCurrentHP(enemy)+GetMagicShield(enemy)+GetDmgShield(enemy) < CalcDamage(myHero, enemy, 0, 20+16*GetCastLevel(myHero,_E)+0.2*GetBonusAP(myHero)+0.02*GetMaxMana(myHero) + ExtraDmg) then
