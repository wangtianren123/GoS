...
function CanAttack()
	if LastAttack <= GetTime() then
		return (GetTime() + Latency()  > LastAttack + AnimationTime())
	end
	return false
end
...
function CanMove()
	if LastAttack <= GetTime() then
		return (GetTime() + Latency() > LastAttack + WindUpTime())
	end
end
...
function OrbWalk(target, point)
	...
	elseif CanMove() and Move then
		if not point then
			local OBTarget = GetCurrentTarget()
			if SOW.Mode == 1 or not OBTarget then
...
OnProcessSpell(unit, spell
	...
	elseif unit == myHero and spell.name == AAResets[spell.name] then
