	...
	if IWalkConfig.Harass then
	      local target = 1280, DAMAGE_PHYSICAL)
		    ...
		    if ValidTarget(target, 1280) then
                  ...
                  CastSpell(_Q)
				          local WPred = GetPredictionForPlayer(GetMyHeroPos(),target,GetMoveSpeed(target),2000,250,1280,50,true,true)
...
if CanUseSpell(myHero, _W) == READY and DrawingsConfig.DrawQ then DrawCircle(myHeroPos.x,myHeroPos.y,myHeroPos.z,GetCastRange(myHero,_W),3,100,0xff00ff00) end
end
