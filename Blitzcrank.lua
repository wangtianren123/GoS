	      ...
	      local target = GetTarget(1000, DAMAGE_MAGIC)
	               if ValidTarget(target, 1000) then
		              ...
		              local QPred = GetPredictionForPlayer(GetMyHeroPos(),target,GetMoveSpeed(target),1800,250,925,70,true,true)
                              ...
                              if CanUseSpell(myHero, _W) == READY and not IsInDistance(target, 150) and IsInDistance(target, 400) Config.W then
		              ...
		              local RPred = GetPredictionForPlayer(GetMyHeroPos(),target,GetMoveSpeed(target),0,250,0,550,false,true)
